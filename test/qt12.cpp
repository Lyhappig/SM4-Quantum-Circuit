...
void qt12_inv(int a[4], int b[4], int d[28]) {
	cx(a[3], d[6]);
    cx(a[1], d[6]);
    cx(b[3], d[7]);
    cx(b[1], d[7]);
    ...
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
            ...
            for (int k = 0; k < 4; k++) {
                ...
                if (a[k] != expected_a) {
                    printf("qt12: output X not match\n");
                    return 0;
                }
                if (b[k] != expected_b) {
                    printf("qt12: output Y not match\n");
                    return 0;
                }
            }
			...
			for (int k = 4; k < n; k++) {
				if (d1[k] != 0) {
					printf("qt12: can't recover other bits\n");
                    return 0;
				}
			}
            ...
            for (int k = 0; k < 4; k++) {
                ...
                int expected_b = (j >> (3 - k)) & 1;
                if (a[k] != expected_a) {
                    printf("qt12_inv: output X not match\n");
                    return 0;
                }
                if (b[k] != expected_b) {
                    printf("qt12_inv: output Y not match\n");
                    return 0;
                }
            }
            for (int k = 0; k < n; k++) {
				if (d1[k] != 0) {
					printf("qt12_inv: can't recover other bits\n");
                    return 0;
				}
			}
        }
