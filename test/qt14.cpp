...
int main() {
	for (int i = 0; i < 16; i++) {
		...
		printf("%d %d\n", y1, y2);
		if (c[5] != 0) {
			puts("qt14: can't recover c[5]");
			exit(0);
		}
		if (y1 != y2) {
			puts("qt14: answer doesn't match");
			exit(0);
		}
		qt14_inv(a, c);
		...
		for (int j = 0; j < 6; j++) {
			if (c[j] != 0) {
				puts("qt14_inv: can't recover qubit");
				exit(0);
			}
		}
		if (k != i) {
			puts("qt14_inv: can't recover input");
			exit(0);
		}
	}
	puts("OK");
