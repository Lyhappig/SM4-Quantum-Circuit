...
// 9个QAND_1，29个CNOT
void qt16_inv(int a[4], int d[21]) {
	std::swap(d[9], a[0]);
    std::swap(d[10], a[1]);
    std::swap(d[12], a[2]);
    std::swap(d[13], a[3]);
...
int main() {
	for (int i = 0; i < 16; i++) {
		...
		printf("%d %d\n", y1, y2);
		if (y1 != y2) {
			puts("qt16: answer doesn't match");
			exit(0);
		}
		for (int j = 15; j < 21; j++) {
			if (d[j] != 0) {
				puts("qt16: can't revocer qubit");
				exit(0);
			}
		}
		qt16_inv(a, d);
		...
		for (int j = 0; j < 21; j++) {
			if (d[j] != 0) {
				puts("qt16_inv: can't revocer qubit");
				exit(0);
			}
		}
		if (k != i) {
			puts("qt16_inv: can't revocer input");
			exit(0);
		}
	}
	puts("OK");
